#include <bits/stdc++.h>
// Tymon Tumialis
using namespace std;
typedef long long ll;
constexpr int MAX_VAl = 20;

ll cost_of_move[MAX_VAl];
ll Height_of_element[MAX_VAl];
ll Pref_sum_right[MAX_VAl];
ll Pref_sum_left[MAX_VAl];
ll Right_dominos_which_falls[MAX_VAl];
ll Left_dominos_which_falls[MAX_VAl];

int main()
{
    ios_base::sync_with_stdio(0);
    cin.tie(0);
    cout.tie(0);
    int n, h, h1, h2;
    ll x, n1, n2, res = 0, prev = 0, left_range = 0, index_left = 0, right_range = 0, right_index = 0;
    vector<int> TO_add_left;
    vector<int> To_add_right;
    cin >> n;
    // reading date
    for (int i = 0; i < MAX_VAl; i++)
    {
        Right_dominos_which_falls[i] = -1;
        Left_dominos_which_falls[i] = -1;
    }
    for (int i = 0; i < n; i++)
    {
        cin >> x >> h;

        Height_of_element[i] = h;
        Pref_sum_right[i] = x;
        if (i != 0)
            cost_of_move[i - 1] = x - prev;
        prev = x;
    }
    cin >> n1 >> h1 >> n2 >> h2;
    // Calculating Pref_sum
    for (int i = n - 2; i >= 0; i--)
        Pref_sum_left[i] = Pref_sum_left[i + 1] + cost_of_move[i];

    for (int i = 1; i < n; i++)
    {
        left_range = Pref_sum_left[i] + Height_of_element[i];
        auto itr = lower_bound(Pref_sum_left + n, Pref_sum_left, left_range);
        index_left = *itr;
        for (int j = index_left; j < i; j++)
        {
            if (Left_dominos_which_falls[j] == 0)
                break;
            if (Pref_sum_left[j] - left_range > 0)
                Left_dominos_which_falls[j] = Pref_sum_left[j] - left_range;
            
            else
                Left_dominos_which_falls[j] = 0;
        }
    }

    for (int i = n - 2; i >= 0; i--)
    {
        right_range = Pref_sum_right[i] + Height_of_element[i];
        auto itr = lower_bound(Pref_sum_right, Pref_sum_right + n, right_range);
        right_index = *itr;
        for (int j = right_index; j < i; j++)
        {
            if (Right_dominos_which_falls[j] == 0)
                break;
            if (Pref_sum_right[j] - right_range > 0)
            
                Right_dominos_which_falls[j] = Pref_sum_right[j] - right_range;

            else
                Right_dominos_which_falls[j] = 0;
        }
    }

    return 0;
}