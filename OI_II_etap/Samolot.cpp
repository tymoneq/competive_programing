...
public:
    ...
    inline void solve()
    {
        ...
        for (auto przed : Przedzial)
        {
            ...
            {
                if (it->start < przed.l && it->koniec > przed.r)
                {
                    ll time_difference = timer - (przed.r - przed.l + 1 + it->last_time_used);

                    auto itr = WartosciBezpieczenstwa.upper_bound(time_difference);
                    ...
                    timer += przed.r - przed.l + 1;
                }
                else if (it->start == przed.l && it->koniec == przed.r)
                {
                    ll time_difference = timer - (it->koniec - it->start + 1 + it->last_time_used);
                ...
                {
                    ll time_difference = timer - (przed.l - it->start + 1 + it->last_time_used);
                    ...
                    add(id, it->koniec - przed.l + 1);

                    to_erase.push_back(*it);
                    to_add.push_back({it->last_time_used, it->start, przed.l - 1});

                    timer += it->koniec - przed.l + 1;
                }

                else if (it->start >= przed.l && it->koniec <= przed.r)
                {
                    ll time_difference = timer - it->last_time_used - 1;
                    ...
                    add(id, it->koniec - it->start + 1);

                    to_erase.push_back(*it);

                    timer += it->koniec - it->start + 1;
                }
